...
const HomeListAll = () => {
  ...
  const [latestAnimation, setLatestAnimation] = useState([]);
  const [popularAnimation, setPopularAnimation] = useState([]);
  ...
  const [latestAnimationRef, latestAnimationInView] = useInView({
    triggerOnce: true,
  });
  const [popularAnimationRef, popularAnimationInView] = useInView({
    triggerOnce: true,
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setLatestAnimation(lT.results);
      } catch (error) {
        console.error("Error fetching data:", error);
      }
    };
    if (latestAnimationInView) fetchData();
  }, [latestAnimationInView]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setPopularAnimation(lT.results);
      } catch (error) {
        console.error("Error fetching data:", error);
      }
    };
    if (popularAnimationInView) fetchData();
  }, [popularAnimationInView]);
  ...
  return (
    <div className={styles.HomeListAll}>
      ...
      <div className={styles.HomeListSection}>
        {latestAnimation?.map((ele) => {
          return <MovieCardSmall data={ele} media_type="tv" />;
        })}
        {latestAnimation?.length === 0 &&
          dummyList.map((ele, i) => (
            <Skeleton className={styles.loading} key={i} />
          ))}
      </div>
      <h1 ref={popularAnimationRef}>
        Popular Animation
        <div>
          <MdChevronLeft
            onClick={() => {
      ...
      <div className={styles.HomeListSection}>
        {popularAnimation?.map((ele) => {
          return <MovieCardSmall data={ele} media_type="tv" />;
        })}
        {popularAnimation?.length === 0 &&
          dummyList.map((ele, i) => (
            <Skeleton className={styles.loading} key={i} />
          ))}
      </div> */}
